...
SRC = ROOT / "PTC_QC_Procedures_v1.1_beginner.md"
DST = ROOT / "PTC_QC_Procedures_v1.1_beginner.html"
...
def embed_local_images(html: str, base_dir: Path) -> str:
    """Replace <img src="relative/path"> with data: URIs so one HTML file needs no assets folder."""

    img_re = re.compile(r'<img\s+src="([^"]+)"\s+alt="([^"]*)"\s*/>')
    ...
    def repl(match: re.Match) -> str:
        ...
        return f'<img src="data:{mime};base64,{b64}" alt="{html_lib.escape(alt)}" />'
...
<body>
  <main class="page">
    <div class="meta">Generated from beginner markdown on {generated} · Self-contained: figures embedded (no extra <code>assets/</code> folder needed to view this file).</div>
...
def main():
    ...
    rendered = embed_local_images(rendered, ROOT)
